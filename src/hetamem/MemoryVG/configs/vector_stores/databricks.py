...
class DatabricksConfig(BaseModel):
    ...
    client_id: Optional[str] = Field(
        None, description="Databricks Service principal client ID"
    )
    ...
    warehouse_name: Optional[str] = Field(
        None, description="Databricks SQL warehouse Name"
    )
    ...
    def validate_authentication(self):
        ...
        has_service_principal = (
            self.client_id is not None and self.client_secret is not None
        ) or (self.azure_client_id is not None and self.azure_client_secret is not None)
