...
if __name__ == "__main__":
    ...
    # 2. 初始化 Chunker
    # chunkerify 是工厂函数，用来生成切分器实例
    # 参数 1: 分词器名称 (这里用 GPT-4 的分词器 'cl100k_base')
    # 参数 2: chunk_size (你的要求：1024)
    print("正在初始化 Chunker...")
    chunker = chunkerify(
    ...
    # 4. 查看结果
    print(f"\n处理完成！共切分为 {len(chunks)} 个块。")

    # 保存为 JSONL 格式
    encoding = tiktoken.get_encoding("cl100k_base")
    with open("data/output_chunks_1.jsonl", "w", encoding="utf-8") as f:
        for i, chunk in enumerate(chunks):
            # 核心修改：使用 encoding.encode() 将文本转为 token ID 列表，然后计算列表长度
